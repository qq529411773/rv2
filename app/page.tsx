...
export default function Home() {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        const hasUsedFree =
          localStorage.getItem("hasTriedFreeGeneration") === "true";
  ...
  const handleGenerate = async (formData: FormData) => {
    ...
    if (!user && hasTriedFree) {
      toast({
        title: t.generator.toastFreeTrialUsed,
        description: t.generator.toastFreeTrialUsedDesc,
      });
      router.push("/sign-in");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 && data.rateLimited) {
          toast({
            title: t.generator.toastDailyLimit,
            description:
              data.error ||
              "You can generate 3 free names per day. Please sign in for unlimited access!",
          });
          // Show sign-in option
          setTimeout(() => {
            router.push("/sign-in");
      ...
      toast({
        title: data.message || t.generator.toastNamesGenerated,
